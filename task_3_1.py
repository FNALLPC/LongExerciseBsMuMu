...
def task_3_1():
    ...
    pt_t   = ROOT.std.vector('float')(1)
    eta_t  = ROOT.std.vector('float')(1)
    tin.SetBranchAddress("cate", cate_t.data())
    tin.SetBranchAddress("wgt",  wgt_t.data())
    tin.SetBranchAddress("m",    m_t.data())
    tin.SetBranchAddress("pt",   pt_t.data())
    tin.SetBranchAddress("eta",  eta_t.data())

    for evt in range(tin.GetEntries()):
        tin.GetEntry(evt)
        if cate_t[0] != cate: continue
        if m_t[0] < 5.0 or m_t[0] >= 5.8: continue
        if pt_t[0] < pt_min or pt_t[0] >= pt_max: continue
        if abs(eta_t[0]) < eta_min or abs(eta_t[0]) >= eta_max: continue
        m.setVal(m_t[0])
        wgt.setVal(wgt_t[0])
    ...
    eff       = evtcount[1] / evtcount[0]
    eff_error = math.sqrt(eff * (1. - eff) / evtcount[0])
    fin.Close()
    ...
    rds_mc.plotOn(frame1, ROOT.RooFit.Name("rds_mc"))
    pdf_sigmc.plotOn(frame1, ROOT.RooFit.Name("pdf_sigmc"), ROOT.RooFit.LineWidth(3))
    ...
    rds_data.plotOn(frame2, ROOT.RooFit.Name("rds_data"))
    model.plotOn(frame2, ROOT.RooFit.Name("model"), ROOT.RooFit.LineWidth(3))
    model.plotOn(frame2, ROOT.RooFit.Name("pdf_comb"), ROOT.RooFit.Components("pdf_comb"),
    ...
    canvas2.Print("task3_1b.pdf")

    print("Category:", cate)
    print("pt range: %.1f, %.1f" % (pt_min, pt_max))
    print("|eta| range: %.1f, %.1f" % (eta_min, eta_max))
    print("Selection efficiency: %.5f +- %.7f" % (eff, eff_error))
    print("Observed yield: %.4f +- %.4f" % (n_sig.getVal(), n_sig.getError()))
