...
def build_gaussian_constraint(wspace, varname, mean, sigma):
    vmin = mean - sigma * 6.
    vmax = mean + sigma * 6.
    if vmin < 0.:
        vmin = 0.

    var       = ROOT.RooRealVar(varname,            "", mean, vmin, vmax)
    var_mean  = ROOT.RooRealVar(varname + "_mean",  "", mean)
    var_sigma = ROOT.RooRealVar(varname + "_sigma", "", sigma)
    gau       = ROOT.RooGaussian(varname + "_gau",  "", var, var_mean, var_sigma)
    wspace.Import(gau)


def build_lognormal_constraint(wspace, varname, mean, sigma):
    vmin = mean - sigma * 6.
    vmax = mean + sigma * 6.
    if vmin < 0.:
        vmin = 0.
    ...
    var       = ROOT.RooRealVar(varname,            "", mean, vmin, vmax)
    var_mean  = ROOT.RooRealVar(varname + "_mean",  "", mean)
    var_kappa = ROOT.RooRealVar(varname + "_kappa", "", 1. + sigma / mean)
    lnn       = ROOT.RooLognormal(varname + "_lnn", "", var, var_mean, var_kappa)
    wspace.Import(lnn)
...
def build_pdf_peak(wspace, cate, bdt_min):
    ...
    leg.AddEntry(frame.findObject("t_rds"), "Simulation", "EP")
    leg.AddEntry(frame.findObject("t_pdf"), "PDF",        "L")
    leg.Draw()

    canvas.Print("task6_4a_%d.pdf" % cate)
    ...
    wspace.Import(pdf)
    build_lognormal_constraint(wspace, "N_peak_%d" % cate, sum_weight, sum_weight_err)
...
def build_pdf_semi(wspace, cate, bdt_min):
    ...
    canvas.Print("task6_4b_%d.pdf" % cate)
    ...
    wspace.Import(pdf)
    build_lognormal_constraint(wspace, "N_semi_%d" % cate, sum_weight, sum_weight_err)
...
def build_pdf_bs(wspace, cate, bdt_min):
    ...
    eff_error = effyield[cate][dEff_bsmmMc] / effyield[cate][Eff_bsmmMc] * eff  # ignore MC statistics error
    fin.Close()
    ...
    canvas.Print("task6_4c_%d.pdf" % cate)
    ...
    wspace.Import(pdf)
    build_gaussian_constraint(wspace, "Eff_bs_%d" % cate, eff, eff_error)
...
def fit_bupsik(wspace, cate, bdt_min):
    ...
    canvas1.Print("task6_4d_%d.pdf" % cate)
    ...
    canvas2.Print("task6_4e_%d.pdf" % cate)

    print("Category:", cate)
    print("Selection efficiency: %g +- %g" % (eff, eff_error))
    print("Observed yield: %g +- %g" % (n_sig.getVal(), n_sig.getError()))

    build_gaussian_constraint(wspace, "N_bu_%d" % cate, n_sig.getVal(), n_sig.getError())
...
def task_6_4():
    ...
    build_gaussian_constraint(wspace, "fs_over_fu", 0.252, math.sqrt(0.012**2 + 0.015**2))
